...
public class AnimationBakerWindow : EditorWindow {
	...
	Event curEvent;
	bool area1Clicked = false, area2Clicked = false, isLegacyType = true;
...
*/
	...
	void AssignPreferences(){
		...
		sampleRate = preferences.frameRate;
	}
	...
	void AnimationClipsArea(){
		...
		GUILayout.Label ("Animations list");
	...
	void DrawAnimationClipField(int id){
		...
		EditorGUILayout.LabelField(animationsList[id].clip.name,EditorStyles.boldLabel,GUILayout.Width(120));
	...
	void CharactersArea(){
		...
		GUI.color = Color.white;
		GUILayout.BeginVertical ();
		GUILayout.Label ("Characters list");
		GUI.color = Color.green;
	...
	void MainMenu_RecordingOptionsArea(){
		...
		isLegacyType = EditorGUILayout.ToggleLeft("Mark clip as 'legacy'",isLegacyType);
